...
if __name__ == "__main__":
    ...
    parser.add_argument("--invert_path", type=str)
    ...
    invert_cvs = []
    if args.invert_path is not None:
        invert_dst_names = ["topleft", "topright", "bottomleft", "bottomright"]
        for i in range(len(invert_dst_names)):
            invert_cv = cm.create(
                join(args.invert_path, invert_dst_names[i]),
                data_type=args.coarse_field_dtype,
                num_channels=2,
                fill_missing=True,
                overwrite=False,
            ).path
            invert_cvs.append(invert_cv)    
